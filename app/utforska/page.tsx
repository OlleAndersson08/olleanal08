...
import { mojligheter, TYPER, type TypNyckel } from "@/data/mojligheter";

/*
  Utforska – bläddra och sök bland ALLA kategorier av möjligheter.
  Här bor kategorifiltreringen (flödet hålls rent och kuraterat).
*/
        ...
        <div className="mt-2 flex flex-col gap-3">
          {traffar.map((m) => {
            ...
            return (
              <div
                key={m.id}
                className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 p-3 transition hover:bg-white/[0.08]"
              >
                <span
    ...
    <button
      ...
      className={`whitespace-nowrap rounded-full border px-4 py-2 text-sm font-semibold transition ${
        aktiv
          ...
          : "border-white/10 bg-white/5 text-mute hover:text-text"
      }`}
    >
      {children}
    </button>
